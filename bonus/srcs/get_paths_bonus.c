...
/*   Updated: 2024/07/20 17:08:25 by rkassel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fill_path(t_pipex *pipex, char *full_path_cmd, int i)
{
	int	j;
...
void	get_valid_cmd_paths(t_pipex *pipex)
{
	...
	(init_cmds_path(pipex), i = 0);
	while (pipex->cmds_list[i])
	{
		j = 0;
		if (access(pipex->cmds_list[i][0], X_OK) == 0)
			fill_path(pipex, pipex->cmds_list[i][0], i);
		if (pipex->env_paths)
		{
			while (pipex->env_paths[j])
			{
				...
				if (access(pipex->full_path_cmd, X_OK) == 0)
					j = fill_path(pipex, pipex->full_path_cmd, i);
